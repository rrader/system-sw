    ...
    def quant(self):
        ...
        if random.random() < 0.9 and self.last_page:
            ...
            if vplid not in self.pages:
                self.pages[vplid] = self.VM.request_page(self.pid, random.random() > 0.1)
...
class VM(object):
    ...
    def request_page(self, pid, fs=False):
        ...
        self.virtual_pages[vpid]['flags'] |= IO_BIT if fs else 0
